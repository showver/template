 ...
 * @LastEditTime: 2021-05-10 17:50:50
...
<template>
  <div class="station-images">
    <el-row :gutter="30">
        <el-col :span="4">
            <el-select v-model="tabelForm.pressureSiteId" placeholder="请选择">
                <el-option v-for="(item, index) in siteList" :key="index" :label="item.pressureSiteName" :value="item.id" ></el-option>
            </el-select>
        </el-col>
        <el-col :span="10">
    ...
    <el-table :data="tableData">
      ...
      <el-table-column label="监测站点名称" prop="pressureSiteName"></el-table-column>
      <el-table-column label="图片" prop="rotatePicturesPath">
        <template slot-scope="scope">
          <img :src="scope.row.rotatePicturesPath" width="70" />
        </template>
      </el-table-column>
    ...
    <!-- 模态框 -->
    <el-dialog :title="dialogMap[dialogStatus]" :visible.sync="dialogVisible" width="30%" :show-close="false">
      <el-form ref="dialogForm" :model="dialogForm" :inline="true" label-width="100px" :rules="dialogRules">
        <el-form-item label="站点名称" prop="pressureSiteId">
          <el-select v-model="dialogForm.pressureSiteId" placeholder="">
            <el-option v-for="(item, index) in siteList" :key="index" :label="item.pressureSiteName" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="图片" prop="multipartFile">
          <div class="file-box">
            ...
            <img v-if="dialogForm.rotatePicturesPath" :src="dialogForm.rotatePicturesPath" class="avatar">
...
<script>
import { getSiteInfo, getSiteImg, postSiteImg, putSiteImg, deleSiteImg } from '@/api/site'
  ...
  data() {
    return {
      ...
      dialogForm: {
        ...
        multipartFile: null,
        rotatePictures: null,
        rotatePicturesPath: null
      },
      dialogRules: {
        pressureSiteId: [
          { required: true, message: '请选择监测站点名称', trigger: 'change' }
        ]
      },
    }
  },
  ...
  methods: {
    handleCropper() {
      this.$refs.cropper.getCropData((data) => {
        this.dialogForm.rotatePicturesPath = data
    ...
    handleAdd() {
      ...
      this.dialogForm.rotatePicturesPath = null
      ...
      if (/^image/.test(file.type)) {
        ...
        reader.onloadend = function() {
          ...
          img.onload = () => {
            ...
            // that.dialogForm.rotatePicturesPath = data
      ...
      this.$refs['dialogForm'].validate((valid) => {
        if (valid) {
          if (this.dialogForm.multipartFile) {
            ...
            formData.append('pressureSiteId', this.dialogForm.pressureSiteId)
            formData.append('multipartFile', this.dialogForm.multipartFile)
            postSiteImg(formData).then((data) => {
    ...
    updateData() {
      this.$refs['dialogForm'].validate((valid) => {
        if (valid) {
            ...
            formData.append('rotatePictures', this.dialogForm.rotatePictures)
            putSiteImg(formData).then((data) => {
                this.dialogVisible = false
                this.fetchData()
    ...
    fetchData() {
      getSiteImg(this.tabelForm).then((data) => {
